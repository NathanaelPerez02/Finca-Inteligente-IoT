...
if (isset($_GET['usuario']) && isset($_GET['humedad']) && isset($_GET['agua']) && isset($_GET['acceso']) && isset($_GET['modo'])) {
  ...
  if (mysqli_num_rows($consulta) > 0) {
    ...
    // GUARDA EN HISTORIAL SOLO SI HUMEDAD O AGUA CAMBIARON
    // Comparamos con la última lectura guardada en el historial
    $ultima = mysqli_query($conn, "SELECT humedad, agua FROM historial_lecturas 
                                   WHERE usuario = '$usuario' 
                                   ORDER BY id DESC LIMIT 1");
    $hay_cambio = true; // por defecto asumimos que hay cambio

    if ($ultima && mysqli_num_rows($ultima) > 0) {
      $ult = mysqli_fetch_assoc($ultima);
      // Solo guardamos si al menos uno de los dos valores cambió
    ...
    if ($hay_cambio) {
      $sql_historial = "INSERT INTO historial_lecturas (usuario, humedad, agua, fecha) 
    ...
    // Actualizar valores en tiempo real (siempre, independiente del historial)
